            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(self.boosted_url) as response:
                    if response.status == 200:
                        ...
                        # DexScreener boosted API returns list of objects containing 'tokenAddress'
                        # We map to 'symbol' and 'address'
                        return [{"symbol": item.get('symbol'), "address": item.get('tokenAddress')} for item in data]
                    return []
