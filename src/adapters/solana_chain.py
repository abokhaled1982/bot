...
class SolanaAdapter:
    async def get_top_10_holder_percent(self, token_address: str) -> float:
        try:
            supply_result = await asyncio.to_thread(rpc_call, "getTokenSupply", [token_address])
            ...
            largest = await asyncio.to_thread(rpc_call, "getTokenLargestAccounts", [token_address])
            if not largest:
                return 50.0
