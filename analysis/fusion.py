...
class SignalFusion:
    ...
    def apply_prefilter(self, token_data: dict, chain_data: dict, market_data: dict, msgs: list) -> bool:
        """
        Strategie 1: Zweistufiger Vorfilter.
        NEU: Volume Spike > 3x und mindestens 1 Alpha-Mention in den letzten 30 Minuten.
        """
        symbol = token_data.get("symbol", "UNKNOWN")
        
        # 1. Volume-Spike < 3x? (UPDATE: 3x statt 2x)
        if token_data.get("volume_spike", 0) < 3.0:
            logger.info(f"[{symbol}] PRE-FILTER SKIP: Volume spike < 3x.")
            return False

        # 2. Keine Telegram Alpha-Mentions in letzten 30 Minuten?
        if not msgs or len(msgs) == 0:
            logger.info(f"[{symbol}] PRE-FILTER SKIP: No Alpha-Mentions in the last 30 minutes.")
            return False
            
        # 3. Liquidity Locked?
        if not chain_data.get("liquidity_locked", False):
            logger.info(f"[{symbol}] PRE-FILTER SKIP: Liquidity NOT locked.")
            return False
            
        # 4. Top-10-Holder > 80%?
        if chain_data.get("top_10_holder_percent", 100) > 80:
            logger.info(f"[{symbol}] PRE-FILTER SKIP: Top 10 hold > 80%.")
            return False
            
        # 5. BTC drops > 5% in 1h?
        if market_data.get("btc_1h_change", 0) < -5.0:
            logger.info(f"[{symbol}] PRE-FILTER SKIP: BTC dropped > 5%.")
            return False
